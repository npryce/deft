...
class StorageContract:
    ...
    def test_can_delete_directory_tree(self):
        self.storage.makedirs(["parent", "child"])
    ...
    def test_lists_files_that_match_list_of_glob_patterns(self):
    ...
    def _create_example_file(self, dirlist):
        with self.storage.open_write(dirlist) as output:
            output.write("testing")
...
class FileStorage_Test(StorageContract):
    ...
    def test_files_are_created_on_disk_in_basedir(self):
        with self.storage.open_write(["foo", "bar"]) as output:
            output.write("testing")
        
        
        assert_that(os.path.exists(self.abspath("foo/bar")), equal_to(True))
        assert_that(open(self.abspath("foo/bar"),"r").read(), equal_to("testing"))
